              ...
              <CardContent className="p-0">
                ...
                <form onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    // Store user info in sessionStorage for immediate access during navigation
                    // This is needed because Supabase session might not be persisted to localStorage yet
                    if (data.user) {
                      sessionStorage.setItem('auth_user_temp', JSON.stringify({
                        id: data.user.id,
                        email: data.user.email,
                        timestamp: Date.now()
                      }))
                      console.log('[Auth] Stored user info in sessionStorage')
                    }
                  ...
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      try {
                        ...
                        showMessage('تم تسجيل الدخول بنجاح')
                        const nextParam = searchParams.get('next')
                        // Determine redirect target: for signups on main domain, go to create-store
                        const isMainDomain = typeof window !== 'undefined' && !window.location.hostname.includes('.')
                        const redirectTarget = nextParam
                          ? decodeURIComponent(nextParam)
                          : isMainDomain ? '/create-store' : '/'
                        // small delay so user sees the message then navigate
                        setTimeout(() => {
