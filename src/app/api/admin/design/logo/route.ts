...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!fileExt || !allowedExtensions.includes(fileExt) || !allowedTypes.includes(file.type)) {
      return NextResponse.json(
        { error: "Unsupported file type. Please use JPG, PNG, WebP, or SVG." },
        { status: 400 }
      )
    }
    ...
    // Generate unique file name with store_id prefix for isolation
    const fileName = `${storeId}/${randomUUID()}.${fileExt}`

    // Upload to Supabase Storage bucket "site-logo"
    ...
    // Upsert row in design_settings for this store
